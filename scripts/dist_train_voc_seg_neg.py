...
from model.losses import get_masked_aff_loss, get_seg_loss, MCCLoss, DenseEnergyLoss, get_energy_loss
...
from utils.camutils import cam_to_label, label_to_aff_mask, refine_cams_with_bkg_v2, cam_to_attn_mask, multi_scale_cam
...
parser.add_argument("--mask_rate", default=0.95, type=float, help="mask ratio in mcc")
parser.add_argument("--scale", default=4, type=int, help="scale in mcc")

parser.add_argument("--w_aff", default=0.2, type=float, help="w_aff")
parser.add_argument("--w_mcc", default=0.5, type=float, help="w_mcc")
...
def validate(model=None, data_loader=None, args=None, n_iter=None):
    ...
    with torch.no_grad():
        for _, data in tqdm(enumerate(data_loader), total=len(data_loader), ncols=100, ascii=" >="):
            ...
            _cams, _cams_aux,_,_ = multi_scale_cam(model, inputs, args.cam_scales)
...
def train(args=None):
    ...
    nlocals = 10
    MCC_loss = MCCLoss(nlocals=nlocals, temp=args.temp).cuda()
    ...
    for n_iter in range(args.max_iters):
        ...
        try:
            img_name, inputs, cls_label, img_box, views = next(train_loader_iter)
        ...
        cams, cams_aux,cam_h,cam_w  = multi_scale_cam(model, inputs=inputs, scales=args.cam_scales)
        if n_iter > 12000:
            resized_cams_aux = F.interpolate(cams, size=(cam_h,cam_w), mode="bicubic", align_corners=False)
        else:
            resized_cams_aux = F.interpolate(cams_aux, size=(cam_h,cam_w), mode="bicubic", align_corners=False)

        attn_masks,flags = cam_to_attn_mask(resized_cams_aux.detach(), cls_label, args)

        cls, segs, fmap, cls_aux, out_t,out_s = model(inputs, views=views, n_iter=n_iter, attn_masks = attn_masks)

        cls_loss = F.multilabel_soft_margin_loss(cls, cls_label)
        cls_loss_aux = F.multilabel_soft_margin_loss(cls_aux, cls_label)

        mcc_loss = MCC_loss(out_s, out_t, flags)
        ...
        aff_loss = get_masked_aff_loss(fmap, aff_mask)

        if n_iter <= 2000:
            loss = 1.0 * cls_loss + 1.0 * cls_loss_aux + args.w_aff * aff_loss + args.w_mcc * mcc_loss + 0.0 * seg_loss + 0.0 * reg_loss
        else:
            loss = 1.0 * cls_loss + 1.0 * cls_loss_aux + args.w_aff * aff_loss + args.w_mcc * mcc_loss + args.w_seg * seg_loss + args.w_reg * reg_loss
        ...
        if (n_iter + 1) % args.log_iters == 0:
            ...
            if args.local_rank == 0:
                logging.info("Iter: %d; Elasped: %s; ETA: %s; LR: %.3e; cls_loss: %.4f, cls_loss_aux: %.4f, aff_loss: %.4f, mcc_loss: %.4f, seg_loss: %.4f..." % (n_iter + 1, delta, eta, cur_lr, avg_meter.pop('cls_loss'), avg_meter.pop('cls_loss_aux'), avg_meter.pop('aff_loss'), avg_meter.pop('mcc_loss'), avg_meter.pop('seg_loss')))
        ...
        if (n_iter + 1) % args.eval_iters == 0:
            ...
            val_cls_score, tab_results = validate(model=model, data_loader=val_loader, args=args, n_iter=n_iter)
            if args.local_rank == 0:
