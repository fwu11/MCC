...
def train(args=None):
    ...
    for n_iter in range(args.max_iters):
        ...
        resized_cams = F.interpolate(cams_aux, size=(cam_h,cam_w), mode="bicubic", align_corners=False)

        attn_masks,flags = cam_to_attn_mask(resized_cams.detach(), cls_label, args)
