...
class Attention(nn.Module):
    ...
    def attention_with_mask(self, attn, policy, inf=1e12):
        # mask key
        B, N = policy.size()
        attn_policy = policy.reshape(B, 1, 1, N) 
        attn_policy = attn_policy.expand(-1,-1,N,-1)

        # apply attention mask
        masked_attn = attn - attn_policy * inf
        return masked_attn
    ...
    def forward(self, x, policy=None):
        ...
        if policy is not None:
            attn = self.attention_with_mask(attn, policy)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, vis=False, aux_layer=4):
        super().__init__()
    ...
    def forward_features(self, x, policy=None):
