...
class network(nn.Module):
    def __init__(self, backbone, num_classes=None, pretrained=None, init_momentum=None, aux_layer=None):
        ...
        self.proj_head = Projector(in_dim=self.encoder.embed_dim, out_dim=1024)
        self.proj_head_t = Projector(in_dim=self.encoder.embed_dim, out_dim=1024,)
    ...
    def forward_proj(self, views, batch_size, n_iter=None, attn_masks=None):
        global_view = views[:2]
        local_view = views[2]

        num_views = len(attn_masks)
        local_view = local_view.unsqueeze(0).expand(num_views,-1, -1,-1,-1).contiguous()
        local_inputs = local_view.view(-1,*local_view.shape[2:])

        policy = torch.cat(attn_masks,dim=0).view(num_views*batch_size,-1)
        policy = F.pad(input=policy, pad=(1, 0, 0, 0), mode='constant', value=0)
        ...
        local_output_s = self.encoder.forward_features(local_inputs, policy)[0]
    ...
    def forward(self, x, cam_only=False, views=None, n_iter=None,attn_masks=None):

        cls_token, _x, x_aux = self.encoder.forward_features(x)

        batch_size = _x.shape[0]
        if views is not None:
            output_t, output_s = self.forward_proj(views, batch_size, n_iter, attn_masks)
