...
def _validate(model=None, data_loader=None, args=None):
    ...
    with torch.no_grad(), torch.cuda.device(0):
        ...
        for idx, data in tqdm(enumerate(data_loader), total=len(data_loader), ncols=100, ascii=" >="):
            ...
            valid_cam = cls_label_rep * _cams
            inputs_denorm = imutils.denormalize_img2(inputs.clone())
            refined_pseudo_label = refine_cams_with_bkg_v2(par, inputs_denorm, cams=valid_cam, cls_labels=cls_label,  high_thre=0.7, low_thre=0.25, ignore_index=args.ignore_index, img_box=None, ).cpu().numpy()

            alpha = 0.6
            cam_rgb = alpha*cam_rgb + (1-alpha)*cam_rgb
            ...
            imageio.imsave(os.path.join(cam_aux_dir, name[0] +"_pesudo.jpg"), imutils.encode_cmap(np.squeeze(refined_pseudo_label)).astype(np.uint8))
