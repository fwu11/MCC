...
class COCOSegmentation(object):
    ...
    def __init__(self, root_dir, split='train', year='2017', to_voc12=False):
        ...
        self.img_dir = os.path.join(root_dir, 'JPEGImages/{}{}'.format(split, year))
        self.split = split
...
def generate_pairs(coco, save_dir_mask):
    for i in tqdm(range(len(coco.ids))):
        fname, mask = coco.generate_img_mask_pair(i)
        if fname is None:
            continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--year", help="the desired coco dataset year.", type=str, default='2014')
    parser.add_argument("--to-voc12", help="wheater to convert to pascalvoc class set.", type=lambda x: (str(x).lower() == 'true'), default=False)
    parser.add_argument("--coco-path", help="Path to coco dataset.", type=str, required=True)
    ...
    mask_dir_name = 'SegmentationClass/'+ str(split)+str(year)
    ...
    generate_pairs(coco, save_dir_mask=save_dir_mask)
